...
class Tour extends MY_Controller{
    ...
    //------------
    //---no login
    function uGetTourList($seg=1){
        $where = array("mark_draft" => 0);
        ...
        $get_tour = $this->Mdl_tour->getTour($where,$per_page,$start)->result();
    ...
    function modSaveTour(){
        ...
        $se_keyword = $this->input->post("keyword");
        $se_url = $this->input->post("keyurl");
        if(!$se_url):
            $se_url = site_url("tour/detail/{$t_id}");
        endif;  
        $se_des = $this->input->post("keydes");
    ...
    function modDelTour($id){
        ...
        //---delete tour
        $this->Mdl_tour->delTour(array("t_id" => $id));

        //--delete key
        $se = $this->Mdl_seo->delTag(array("kw_id" => $kw_id));

        $this->o_put["msg"] = "Success : Data is deleted";
        $this->output->set_output(json_encode($this->o_put));
    ...
    function seoAutoSave($id){
        $t_name = $this->input->post("tour_title");
        ...
        $get_tour = $this->Mdl_tour->seoGetTour($t_key)->result();
    ...
    function admin(){
        ...
        $this->data["meta_title"] = "Manage tour program";
